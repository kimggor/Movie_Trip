...
export default function Popup({ title, desc, onClick, closePopup }: { title: string; desc: string; onClick: () => void, closePopup: () => void }) {
    return (
        <div className="w-[335px] h-[182px] flex flex-col bg-white rounded-lg shadow-xl relative p-4">
            ...
            <button className="w-full h-[38px] bg-[#9356d6] rounded-md flex items-center justify-center mt-12" onClick={onClick}>확인</button>
        </div>
    );
}
